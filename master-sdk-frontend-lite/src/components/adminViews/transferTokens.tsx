...
const TransferTokens = () => {
  ...
  const getUserContracts = useCallback(async () => {
    const response: ContractsResponseType =
      await rairSDK.contracts?.getFactoryList();
    if (response.success) {
      setUserContracts(
        response.contracts
          .map((item) => {
  ...
  const getContractData = useCallback(async () => {
    ...
    if (selectedContract !== 'null') {
      ...
      const response1 = await rairSDK.contracts?.getById({
        id: selectedContract
      });
      ...
      setContractProducts([]);
      setOwnedTokens([]);
      const response2 = await rairSDK.contracts?.getProductsAndOffersById({
        id: selectedContract
      });
  ...
  const getProductNFTs = useCallback(async () => {
    ...
    if (selectedProduct !== 'null') {
      const response4 = await rairSDK.nft?.getByIdAndProduct({
        id: selectedContract,
