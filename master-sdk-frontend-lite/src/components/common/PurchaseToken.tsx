...
const queryRangeDataFromDatabase = async (
  ...
  if (!contractInstance || !network) {
    return;
  }
  const response = await rairSDK.contracts?.findContractAndOffers({
    networkId: network,
    contractAddress: (await contractInstance.getAddress()) as Hex
  });
  if (response?.success) {
    if (diamond) {
      for (const product of response.products) {
        for (const offer of product.offers) {
          if (offer.offerIndex && offerIndex?.includes(offer.offerIndex)) {
            return {
              start: offer.range[0],
              end: offer.range[1],
      ...
      const [selectedOfferPool] = response.products.filter(
